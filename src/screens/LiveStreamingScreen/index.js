...
const token ='007eJxTYPAqffqaRyJ44nI59tn23j1BmxXnz0mfd4LFYE9TwgmHH2sUGAwSU43Mjc1MksySk00sjJISzU0sTIwMkixNDIwSjU2TLdOWpTQEMjIUrvnOwAiFID4rQ05mWWolAwMABhAe5Q==';
const uid = 0;
...
// const appID = 255068545
// const userID = 'testing456'
// const userID1 = 'testing456123'
// const appSign = 'c037910eb12d2ccefba89f41389b0bab17e8d6134589bf5f4af04e61b530b3b4'
let time=0;
const LiveStreamingScreen = ({navigation,route}) => {
  const scrollViewRef = useRef(null);
  // const zego_preview_view = useRef(null);
  // const zego_play_view= useRef(null);
  // const remote_preview_view = useRef(null);
  // const remote_play_view= useRef(null);
...
const desiredCaptureSignalVolume = 100; // Desired audio capture signal volume (0-100)
    ...
    const toggleMicrophone=()=>{
      setIsMicrophoneOn((prevMicrophoneState) => !prevMicrophoneState); // Toggle the microphone state
      agoraEngineRef.current?.enableLocalAudio(!isMicrophoneOn);
      // Enable or disable local audio based on the current microphone state
    }
    ...
    data.map((item,index)=>{
     ...
     if(item1.storeId==item._id){
      if(item1.inUse){
        ...
        setStoreImage(`https://live-stream-lovely.onrender.com/store/${item.storeUrl}`)
        setTimeout(()=>{
...
const onGetCoHostAccept = async()=>{
    ...
    if(response.data.data.length>0){
      response.data.data.map((item,index)=>{
        if(item.user[0]?._id==userData?._id){
          ...
          // ZegoExpressEngine.instance().startPlayingStream("444",{"reactNativeTag": findNodeHandle(remote_play_view.current), "viewMode": 0, "backgroundColor": 0});
        }
      })
    } 
  ...
  console.log("error",err)
    ...
    useEffect(()=>{
         ...
         const init = async () => {
            ...
            if(route.params.isHost){
              ...
              console.log("Response Lve Streaming",raw)
               ...
               setInterval(() => {
                ...
                // onMessageGet(response.data.data._id)
              }, 1000); 
            }
          }
        catch(err){
          ...
          const onSendCoin = async () => {
            ...
            const onClickAdmin = async () => {
              ...
              // "userId": "64970026d667e6bd38c3ebde",
              // "status":true,
              // "liveId": "6494214d46435493825ac570"
                var raw = JSON.stringify({
      ...
      //   zegoLiveStreamRef.current.loginRoom(route.params.randomUserID, 'audience');
      // };

const removeLiveStreaming = async() => {
  const authToken = await AsyncStorage.getItem('token');
  
   const response =await onDeleteLiveStreamingApi(liveId)
   console.log("Response Lve Streaming",response.data)
  //  stopPreview()
   time=0
   navigation.goBack()
...
const removeWatchingUser = async() => {
  ...
  var raw = JSON.stringify({
   ...
   const response =await onRemoveWatchUserApi(raw)
...
const onCameraOn=async()=>{
  ...
  // ZegoExpressEngine.instance().enableCamera(true).then(() => {
  //   console.log('Camera turned on');
  //   setRefresh(!refresh)
  //   setCameraEnable(true)
  //   onClickA()
  // }).catch((error) => {
  //   console.log('Error turning on the camera:', error);
  // });
}

const onCameraOff=async()=>{
  ZegoExpressEngine.instance().enableCamera(false).then(() => {
    console.log('Camera turned off');
   
    setCameraEnable(false)
    // onClickA()
  }).catch((error) => {
    console.log('Error turning off the camera:', error);
  });
}

const onSpeakerOff=async()=>{
  ZegoExpressEngine.instance().isMicrophoneMuted(false).then(() => {
    console.log('Camera turned off');
    setRefresh(!refresh)
...
const onSpeakerOn=async()=>{
  ZegoExpressEngine.instance().isMicrophoneMuted(true).then(() => {
    console.log('Camera turned off');
    setRefresh(!refresh)
    setSpeakerEnable(!speakerEnable)
    // onClickA()
...
const onBeautifyCam=async()=>{
  ZegoExpressEngine.instance().enableBeautify(1).then(() => {
    console.log('Camera turned off');
...
console.log("Request Message:",Messages)
 let agoraMessages= messagesList
 console.log("agoraMessages:",Messages)
 setComment('')
 agoraMessages.push(Messages)
    const data = JSON.stringify(agoraMessages);
    console.log("data:",data)
    if (agoraEngineRef.current) {
      agoraEngineRef.current.sendStreamMessage(0, data, data.length);
    }
    console.log("agoraMessages:",agoraMessages)
    // messagesList.push(Messages)
    setMessageList(agoraMessages) 

}

const onMessageGet = async(id) => {
try{
  const sendMessage = await onGetMessageApi(id)

  let data=sendMessage.data.data
...
const AcceptPendingRequest = async(item) => {
  ...
  //  ZegoExpressEngine.instance().startPlayingStream("333",{"reactTag": findNodeHandle(remote_play_view.current), "viewMode": 0, "backgroundColor": 0});
  //  console.log("Response Lve Streaming",response.data)
...
return(
 ...
 {/* {isCoHost &&
  ...
  <RtcSurfaceView canvas={{ uid:0 }} style={[styles.cohostVideoView]} />
...
{callingList.length > 0 && isJoined && isHost ? (
        ...
        <View style={styles.subHeaderView}>
          <View style={{
            ...
            flexDirection:'row'
             ...
             {route.params.isHost?
             <>
             <Image style={{width:50,height:50,borderRadius:50}} source={{uri:`https://live-stream-lovely.onrender.com/profile_images/${userData?.photo}`}} />
            ...
            :
            <>
             <Image style={{width:50,height:50,borderRadius:50}} source={{uri:`https://live-stream-lovely.onrender.com/profile_images/${route.params?.userItem?.user[0]?.photo}`}} />
             <View style={{ marginLeft:wp(1),width:wp(23)}}>
            ...
            height:38,
            width:38,
            backgroundColor:'rgba(108, 9, 73,0.5)',
            borderRadius:38,
            alignItems:"center",
            justifyContent:"center"
            }}><Image style={{width:wp(4),height:hp(4),resizeMode:'contain'}} source={plus} /></View>
          </View>
          <View style={{
            flexDirection:'row',
            alignItems:'center',
            ...
            <TouchableOpacity onPress={()=>{setProfileModal(true),setProfileData(watchList?.LiveUser[0].user_data)}}>
             <Image style={{width:40,height:40,borderRadius:40,borderWidth:2,borderColor:'#FEC105'}} source={{uri:`https://live-stream-lovely.onrender.com/profile_images/${watchList?.LiveUser[0].user_data.photo}`}} />
...
{watchList?.LiveUser.length>1?
            <TouchableOpacity style={{marginLeft:wp(2)}} onPress={()=>{setProfileModal(true),setProfileData(watchList?.LiveUser[1].user_data)}}>
              ...
              <Image style={{width:40,height:40,borderRadius:40,borderWidth:2,borderColor:'#fff'}} source={{uri:`https://live-stream-lovely.onrender.com/profile_images/${watchList?.LiveUser[1].user_data.photo}`}}  />
...
{watchList?.LiveUser.length>2?
            <TouchableOpacity style={{marginLeft:wp(2)}} onPress={()=>{setProfileModal(true),setProfileData(watchList?.LiveUser[2].user_data)}}>
              <Image style={{width:40,height:40,borderRadius:40,borderWidth:2,borderColor:'#F6A164'}} source={{uri:`https://live-stream-lovely.onrender.com/profile_images/${watchList?.LiveUser[2].user_data.photo}`}}  />
...
:null}
            ...
            <Image style={{width:wp(7),height:hp(7),resizeMode:'contain',marginLeft:wp(2)}} source={cancelButton} />
            </TouchableOpacity>
          </View>
        </View>
        ...
        renderItem={({item,index})=>{
          return(
               <View style={{
                  ...
                  <Image source={{uri:`https://live-stream-lovely.onrender.com/profile_images/${item.userPhoto}`}} style={{width:30,height:30,borderRadius:30}} />
                  </TouchableOpacity>
  ...
  marginHorizontal:wp(4)
       ...
       {comment.length<=0?
        <Image style={{width:wp(6),height:hp(6),resizeMode:"contain"}} source={commentIcon} />
:null}
        <TextInput 
        value={comment}
        onChangeText={(text)=>{setComment(text)}}
        style={{width:wp(43),marginLeft:wp(1),color:'#fff',fontSize:hp(1.8)}}
        />
        {comment.length>0?
...
:null}
        ...
        <Image source={list2} style={{width:wp(7),height:hp(5),resizeMode:'contain',marginLeft:wp(3)}} />
        </TouchableOpacity>
}
        ...
        <Text style={{fontSize:hp(1.8),color:'#fff',fontWeight:'600',marginLeft:wp(1)}}>Join</Text>
        </TouchableOpacity>
}
...
<View style={{width:'98%',height:hp(3),alignSelf:'center',flexDirection:'row',justifyContent:'space-between'}}>
     ...
     <TouchableOpacity style={{backgroundColor:giftOptionSelect=='Custom'?'#1877F2':'rgba(0,0,0,0.95)',width:"20%",height:hp(3),alignItems:'center',justifyContent:'center'}} onPress={()=>{setGiftOptionSelect('Custom')}}>
      ...
      <View style={{width:'98%',height:hp(5),alignSelf:'center',backgroundColor:"red"}}>
        <FlatList 
          ...
          if(userData?._id!=item.userId){
            return(
              <TouchableOpacity style={{marginLeft:wp(2)}} onPress={()=>{setCoinSendId(item.userId)}}>
                ...
                }} source={{uri:`https://live-stream-lovely.onrender.com/profile_images/${item.user_data.photo}`}} />
              </TouchableOpacity>
            )
            }
        }}
            ...
            renderItem={({item,index})=>{
              return(
                <View style={{
                  ...
                  <TouchableOpacity >
                  <Image source={{uri:`https://live-stream-lovely.onrender.com/profile_images/${item.user[0].photo}`}} style={{width:50,height:50,borderRadius:50}} />
                  </TouchableOpacity>
...
{kickOutModal?
      ...
      statusBarTranslucent
      onRequestClose={()=>{setKickOutModal(false)}}>
      <View style={styles.centeredView}>
...
:null}
          ...
          <Image style={{width:90,height:90,borderRadius:90}} source={{uri:`https://live-stream-lovely.onrender.com/profile_images/${profileData?.photo}`}} />
           </View>
