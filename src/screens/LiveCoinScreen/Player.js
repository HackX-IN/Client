      ...
      {rankingData && rankingData.length > 0 ? (
        rankingData.map((item, index) => (
          <View key={index} style={item.name === 'Player 1' ? styles.highlightedPlayer : null}>
            <View style={{ padding: 10, marginTop: item.name === 'Player 1' ? -30 : -15 }}>
            <Text style={{ color: 'black', fontWeight: 'bold',textAlign:"center",fontSize:hp(2.2) }}>{index+1}</Text>
              <ImageBackground source={index==0 ?frame2:index==1?frame1:frame3} style={{width:wp(50),height:hp(16),alignItems:'center',justifyContent:'center'}} resizeMode='contain'>
              <Image
                source={item.image ? item.image : require('../../assets/profilePic.png')}
                style={{width:100,height:100,resizeMode:"contain",marginTop:hp(5)}}
              />
              </ImageBackground>
              <View style={{ flexDirection: 'column', justifyContent: 'center', alignItems: 'center', marginTop: 5 }}>
                <Text style={{ color: 'black', fontWeight: 'bold',textAlign:"center" }}>{item.name}</Text>
                <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginTop: 2 }}>
                  <Image source={coin} style={{ width: 20, height: 20,resizeMode:'contain' }} />
                  <Text style={{ color: '#1877F2', padding: 5, marginLeft: 5,fontWeight:"600" }}>{item.totalCoinsSent}</Text> 
