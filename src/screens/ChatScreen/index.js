...
const makeLongPress = (user, messages, setMessages) => (context, message) => {
...
const deleteMessage = (buttonIndex, setMessages, messages, message, user) => {
  ...
  db.collection("rooms")
    ...
    .set(
      { deletedFor: firestore.FieldValue.arrayUnion(user.id) },
      { merge: true }
    )
    .catch((err) => {
      console.log(err);
  ...
  if (messages[0]._id === message._id) {
    db.collection("rooms")
      ...
      .set(
        {
          [`last_msg`]: messages.length > 1 ? messages[1].text : "",
          [`last_msg`]: messages.length > 1 ? messages[1].user.name : "",
...
const ChatScreen = ({ navigation, route }) => {
  ...
  const onLongPress = makeLongPress(user, messages, setMessages);
    ...
    <View style={styles.container}>
      <View style={styles.bannerView}>
        <Image
          ...
          source={{
            uri: `http://13.233.229.68:8008/profile_images/${rcvrpic}`,
          }}
        />
        <Text
          style={{
      ...
      <GiftedChat
        ...
        renderActions={(props) => (
          <TouchableOpacity
            ...
            onPress={() => selectFile(props)}
          >
            <Image
              style={{ width: wp(6), height: hp(4), resizeMode: "contain" }}
              source={photo}
            />
          </TouchableOpacity>
        )}
        ...
        renderInputToolbar={(props) => {
          return <InputToolbar {...props} textInputStyle={styles.inputText} />;
        }}
      />
    </View>
  );
